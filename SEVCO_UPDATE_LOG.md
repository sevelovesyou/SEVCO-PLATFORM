...
---
title: CMD Domains tab: manage all SEVCO domains as cards (Executive+)
---
# Task #171 — Domains Management Tab in CMD
...
## Task — music-detail-pages-stream-counts
> Merged: 2026-04-02
...
---
title: News — free RSS aggregator with DB cache (replace X-only source with RSS + optional Tavily)
---
# News — Free RSS Aggregator with DB Cache

## What & Why
The current news system fetches exclusively from the X/Twitter API on every page load, with only in-memory caching (lost on restart). There's no database caching, no RSS ingestion, and no cost-free fallback. This task replaces the main article source with free public RSS feeds (Reuters, BBC, TechCrunch, The Verge, Wired, Ars Technica, CNN, NPR, The Guardian, NY Times, etc.), stores results in a `news_items` DB table, and runs a background refresh every 18 minutes. Tavily is used as an optional boost (max 3 calls/day) if `TAVILY_API_KEY` is set. X API remains only for the social buzz `/api/news/x-feed` section (unchanged, existing code).

## Done looks like
- `/news` page loads instantly — articles served from DB with no live API call on page visit.
- Articles have real titles, summaries, images, and publish dates from actual RSS feeds.
- Background aggregator runs every 18 minutes silently in the process.
- Items older than 6 hours are pruned automatically on each refresh.
- Optional Tavily boost: if `TAVILY_API_KEY` env var is set, 2-3 calls are made per day spread across top categories.
- X API (social buzz) still works via the existing `/api/news/x-feed` endpoint, unchanged.
- Total cost: $0 (RSS is free, Tavily free tier = 1000 credits/month, we use ~60-90).
- No npm packages need to be installed (`fast-xml-parser` is already in the project).

## Database: `news_items` table (`shared/schema.ts`)
...
export const newsItems = pgTable("news_items", {
  ...
  categoryId: integer("category_id"),  // FK not enforced (category may be deleted)
  categoryQuery: text("category_query"), // the category's query string for fast lookup
  sourceType: text("source_type").notNull().default("rss"), // "rss" | "tavily" | "x"
...
Add to startup migration in `server/index.ts`:
```sql
...
CREATE UNIQUE INDEX IF NOT EXISTS news_items_link_idx ON news_items (link);
CREATE INDEX IF NOT EXISTS news_items_cat_query_idx ON news_items (category_query);
CREATE INDEX IF NOT EXISTS news_items_fetched_at_idx ON news_items (fetched_at);
```

## RSS Feed Sources

Define in `server/news-aggregator.ts`:
```typescript
// Category keyword → list of RSS feed URLs to pull from
...
// Map a category query string to keyword buckets for feed selection
function getFeedsForQuery(query: string): string[] {
  ...
  return [...selected].slice(0, 4); // max 4 feeds per category to keep it fast
}
```

## RSS Parsing (`server/news-aggregator.ts`)

Use `fast-xml-parser` (already installed) to parse RSS XML:
...
function parseRssItems(xml: string, sourceName: string): ParsedArticle[] {
  ...
  return items.slice(0, 15).map((item: any) => {
    ...
    const description = stripHtml(item.description ?? item.summary ?? item["content:encoded"] ?? "").slice(0, 500);
    const pubDate = item.pubDate ?? item.published ?? item.updated ?? null;
    const imageUrl = extractImage(item);
    return { title, link: String(link), description, pubDate: pubDate ? new Date(pubDate) : null, source: sourceName, imageUrl };
  }).filter(a => a.title && a.link);
}

// Extract image from <media:content>, <media:thumbnail>, <enclosure>, or description HTML
function extractImage(item: any): string | null {
  ...
  // Try to extract first <img> src from description HTML
...
## Tavily Integration (optional, `server/news-aggregator.ts`)

```typescript
// In-memory rate limiting: max 3 Tavily calls per calendar day
...
async function fetchTavilyArticles(query: string): Promise<ParsedArticle[]> {
  ...
  const res = await fetch("https://api.tavily.com/search", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ api_key: process.env.TAVILY_API_KEY, query, search_depth: "basic", max_results: 10, include_images: true }),
  });
  if (!res.ok) return [];
...
## Background Aggregator Loop (`server/news-aggregator.ts`)

```typescript
const REFRESH_INTERVAL_MS = 18 * 60 * 1000; // 18 minutes
...
async function refreshNewsCache(): Promise<void> {
  ...
  // Prune old items (older than 6 hours)
  await db.delete(newsItems).where(lt(newsItems.fetchedAt, new Date(Date.now() - ARTICLE_TTL_HOURS * 60 * 60 * 1000)));

  const tavilyCategories = categories.slice(0, 3); // only top 3 get Tavily boost
  ...
  for (const cat of categories) {
    ...
    await Promise.allSettled(feedUrls.map(async (url) => {
      try {
        const res = await fetch(url, { signal: AbortSignal.timeout(8000), headers: { "User-Agent": "SEVCO News Aggregator/1.0" } });
        if (!res.ok) return;
    ...
    // 2. Optional Tavily boost (only for first 3 categories, max 3 calls/day total)
    ...
    // 3. Deduplicate by link, then upsert into DB
    const seen = new Set<string>();
    const unique = allArticles.filter(a => { if (seen.has(a.link)) return false; seen.add(a.link); return true; });

    for (const article of unique.slice(0, 40)) {
      await db.insert(newsItems).values({
        ...
        set: { title: article.title, description: article.description ?? null, imageUrl: article.imageUrl ?? null, fetchedAt: new Date() },
...
export function startNewsAggregator(): void {
  ...
  setInterval(() => refreshNewsCache().catch(e => console.error("[news-aggregator] Refresh error:", e)), REFRESH_INTERVAL_MS);
}
```
...
searchNewsItems(query: string, limit: number): Promise<NewsItem[]>;
```
...
**`GET /api/news/feed`** — replace live X API call with DB lookup + X fallback:
```typescript
app.get("/api/news/feed", async (req, res) => {
  ...
  // 2. Try DB text search (for general search queries)
  const searchResults = await storage.searchNewsItems(query, limit);
  if (searchResults.length > 0) {
    return res.json(searchResults.map(articleToNewsArticle));
  }

  // 3. Fall back to live X API (if configured) for unknown queries
  const xArticles = await fetchNewsArticles(query, limit).catch(() => []);
  res.json(xArticles);
});
```

Where `articleToNewsArticle(item: NewsItem): NewsArticle` maps DB row to `NewsArticle` shape:
```typescript
function articleToNewsArticle(item: NewsItem): NewsArticle {
  return {
    title: item.title,
    link: item.link,
...
app.get("/api/news/feed/all", async (_req, res) => {
  ...
  ...
  res.json(interleaved.map(articleToNewsArticle));
});
```
...
// After DB migrations run:
startNewsAggregator();
```

## `server/news.ts` — Type extension

Change `sourceType` from `"x"` only to `"rss" | "tavily" | "x"`:
```typescript
sourceType?: "rss" | "tavily" | "x";
```

Also extend the `NewsArticle` type in `client/src/components/news-article-card.tsx` the same way (currently `sourceType?: "x"`).

## Files to change
- `shared/schema.ts` — add `newsItems` table
- `server/index.ts` — startup migration SQL + call `startNewsAggregator()`
- `server/news-aggregator.ts` — NEW FILE: RSS fetcher, parser, Tavily client, background loop
- `server/storage.ts` — `getNewsFeedItems`, `searchNewsItems`
- `server/routes.ts` — `GET /api/news/feed` and `GET /api/news/feed/all` serve from DB
- `server/news.ts` — extend `sourceType` type
- `client/src/components/news-article-card.tsx` — extend `sourceType` type (minor)

## What does NOT change
- `/api/news` (categories) — unchanged
- `/api/news/x-feed` — unchanged (X social buzz, existing code)
- `/api/news/categories/*` — unchanged (admin CRUD)
- `/api/news/grok/*` — unchanged (AI features)
- `/api/news/breaking`, `/api/news/summary`, etc. — unchanged
- `news-page.tsx` — no frontend changes needed
- All bookmark, preference, and AI settings endpoints — unchanged
