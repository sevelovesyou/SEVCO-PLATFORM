...
function CardSkeleton() {
  return <div className="w-full h-48 rounded-xl bg-muted animate-pulse" />;
}

function RowSkeleton() {
  return <div className="w-full h-8 rounded-lg bg-muted animate-pulse" />;
}
...
export function NewsBentoGrid({ articles, accentColor, categoryLabel, onArticleClick }: NewsBentoGridProps) {
  ...
  const withImage = articles.filter(articleHasImage);
  const withoutImage = articles.filter((a) => !articleHasImage(a));

  const cardClick = (article: NewsArticle) => onArticleClick ? () => onArticleClick(article) : undefined;

  const [card1, card2, card3, card4, card5, card6, card7, card8] = withImage;
  const fallbackCards = withoutImage;

  return (
    <div data-testid="news-bento-grid">
      {withImage.length > 0 && (
      ...
      {fallbackCards.length > 0 && (
        <div className={`${withImage.length > 0 ? "mt-4 border-t pt-4" : ""} divide-y divide-border`}>
          {fallbackCards.map((article) => (
            <NewsArticleCard
              key={article.link}
              article={article}
              variant="list"
