  ...
  return (
    <div className="flex h-[calc(100vh-3rem)] overflow-hidden">
      ...
      <div className={`flex-1 flex flex-col min-w-0 ${mobileShowEditor ? "flex" : "hidden md:flex"}`}>
        {selectedNote ? (
          <>
            {/* Editor toolbar — Row 1: back, color picker, action buttons */}
            <div className={`flex items-center gap-2 px-3 py-1.5 border-b border-border shrink-0 ${color.bg}`}>
              {/* Mobile back */}
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
              ...
              {/* Desktop toolbar — inline, hidden on mobile */}
              <div className="hidden md:flex items-center flex-1 min-w-0 overflow-x-auto px-1">
                <RichTextToolbar
                  editor={noteEditor}
            ...
            {/* Editor toolbar — Row 2: scrollable formatting toolbar (mobile only) */}
            <div className={`md:hidden flex items-center overflow-x-auto px-2 py-1 border-b border-border shrink-0 [&::-webkit-scrollbar]:hidden [scrollbar-width:none] ${color.bg}`}>
              <RichTextToolbar
                editor={noteEditor}
                uploading={noteUploading}
                onImageButtonClick={imageUploadTrigger ?? undefined}
            ...
            {/* Editor body */}
            <div className={`notes-editor flex flex-col flex-1 overflow-hidden ${color.bg}`}>
