...
| `/messages` | Messages — Email (threads/messages), Chat | None |
...
- 2026-03-29: Home page redesign (glassmorphism + showstoppers) — Full hero redesign with aurora blobs (red/indigo), floating frosted-glass preview cards, word-reveal headline. 3 new showstopper sections: Store (dark red aurora, glassmorphic product cards), Services (dark blue aurora, split-layout with animated service icon grid), Projects (dark green aurora, bento-grid with status badges). RECORDS spotlight upgraded with animated CSS equalizer + Spotify/Apple Music pills. News Teaser strip (3 headlines from /api/news/feed/all). Sign-up CTA panel with feature checklist (hidden for logged-in users). Community CTA glassmorphic upgrade. All sections use IntersectionObserver fade+slide-up. 4 new CMD section toggle keys added to command-settings.tsx SECTION_KEYS: section.servicesShowstopper.visible, section.projectsShowstopper.visible, section.newsTeaser.visible, section.signupCta.visible. 8 new platform Wiki articles seeded via wikiSeed.ts (general category): getting-started, platform-overview, store-shopping-guide, services-guide, projects-ventures-guide, records-music-guide, news-ai-guide, wiki-guide.
- 2026-03-29: Tools marketing page + nav updates — jszip installed (fixes Wikify crash); new /tools page (dark glassmorphism, 5 tool cards with access badges and role-aware CTAs); ToolsDropdown always visible (removed user && gate); ToolsDropdown shows all 5 tools with lock icons for restricted roles; click handler: guests→/auth, wrong role→toast, correct→navigate; mobile nav tools section ungated with same 5 tools and logic; "Wikify Tool" renamed to "Wikify" everywhere (platform-header.tsx HomeDropdown items, homeItems, wikify-tool-page.tsx h1 and description text, app-sidebar.tsx).
- 2026-04-01: Email threaded conversation view — GET /api/email/threads endpoint groups emails by threadId; inbox/starred default to thread view with toggle to flat messages; EmailThreadView component with collapsible older messages, expandable individual messages, and inline reply composer; sendEmail updated to accept threadId param; POST /api/email/send sets threadId on replies.
