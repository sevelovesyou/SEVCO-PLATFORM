...
export async function registerRoutes(
  ...
  app.get("/api/platform-history", async (req, res) => {
    try {
      // Task #517 — /platform reads from the same source as /changelog
      // (the changelog table) so the two pages cannot drift. Where a row has
      // a wikiSlug pointing at a real wiki article, the entry deep-links;
      // otherwise it renders without a link.
      const limitParam = req.query.limit ? parseInt(req.query.limit as string) : null;
      const rows = await storage.getChangelog();
  ...
  // Task #519 — Health endpoint so future drift between /platform,
  // /command/changelog, and /wiki/engineering/sevco-platform is caught
  ...
  app.get("/api/platform-health", requireAuth, requireRole("admin", "executive", "staff"), async (_req, res) => {
    ...
    try {
      const [{ count: changelogRows }] = (
        await db.execute(
          sql`SELECT COUNT(*)::int AS count FROM changelog WHERE wiki_slug LIKE 'platform-task-%'`,
        )
      ).rows as Array<{ count: number }>;
      const [{ count: platformArticles }] = (
        await db.execute(
          sql`SELECT COUNT(*)::int AS count FROM articles WHERE slug LIKE 'platform-task-%'`,
        )
      ).rows as Array<{ count: number }>;
      const missing = (
        await db.execute(
          sql`SELECT c.wiki_slug AS slug, c.title
                FROM changelog c
  ...
  app.post("/api/posts/:id/spark", requireAuth, async (req: any, res) => {
    try {
      ...
      if (result.insufficientFunds) return res.status(402).json({ message: "Not enough Sparks — top up to keep sparking." });
  ...
  function makeSparkHandler(fn: (id: number, userId: string) => Promise<{ alreadySparked: boolean; rateLimited: boolean; selfSpark: boolean; insufficientFunds: boolean }>) {
